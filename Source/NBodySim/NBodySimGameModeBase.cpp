...
ANBodySimGameModeBase::ANBodySimGameModeBase() 
{
    ...
    // // // MegaMesh = NewObject<UInstancedStaticMeshComponent>(this);
    // MegaMesh = CreateDefaultSubobject<UInstancedStaticMeshComponent>(TEXT("MegaMesh"));
    // MegaMesh->RegisterComponent();

    // static ConstructorHelpers::FObjectFinder<UStaticMesh> MeshAsset(TEXT("StaticMesh'/Game/Assets/CircleMeshSimple'"));
    // MegaMesh->SetStaticMesh(MeshAsset.Object);
    
    // // MegaMesh->SetFlags(RF_Transactional);
    // // this->AddInstanceComponent(MegaMesh);
    // RootComponent = MegaMesh;
    // // MegaMesh->SetupAttachment(RootComponent);
    // // MegaMesh->AttachToComponent(RootComponent, FAttachmentTransformRules());
    // FRotator no_rotation(0.0f);
    ...
    //     );
    //     float scale = FMath::FRandRange(10.0, 100.0) * 0.1;
    //     FTransform transform(
    //         no_rotation,
    //         AMass::LocationFrom2DPosition(random_position),
    //         FVector(scale, scale, 1.0)
    //     );
    //     MegaMesh->AddInstance(transform);
    // }
...
void ANBodySimGameModeBase::Tick(float DeltaSecs)
{
	...
	// if(GEngine) GEngine->AddOnScreenDebugMessage(-1, 105.0f, FColor::Green, TEXT("Tock!"));
    // MegaMesh->
